...
class Presensi extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $data['data_nama_kelas']  = $this->PresensiModel->get_nama_kelas($yes->tahun, $yes->semester, $this->session->userdata('iddosen'), false);
    ...
    public function mahasiswa()
    {
        ...
        $this->load->view('layouts/header');
        $this->load->view('layouts/sidebar');
        $this->load->view('presensi/mahasiswa', $data);
        $this->load->view('layouts/footer');
    }
    ...
    public function tampil_absen($idpenyelenggaraan, $idkelas)
    {
        ...
        $this->session->set_userdata('idkelas', $idkelas);
        $yes                           = $this->PresensiModel->get_yes();
        $data['sql_jadwal_dosen']      = $this->PresensiModel->get_jadwal_dosen($yes->tahun, $yes->semester, $this->session->userdata('iddosen'));
        $data['data_ruangan']          = $this->PresensiModel->get_ruang();
        $data['data_hari']             = $this->PresensiModel->get_hari($yes->tahun, $yes->semester, $this->session->userdata('iddosen'));
        $data['data_matakuliah']       = $this->PresensiModel->get_matakuliah($yes->tahun, $yes->semester, $this->session->userdata('iddosen'));
        $data['data_nama_kelas']       = $this->PresensiModel->get_nama_kelas($yes->tahun, $yes->semester, $this->session->userdata('iddosen'), $idkelas);
    ...
    public function tampil_kelompok($nama_kelas)
    {
        ...
        $data['data_hari']        = $this->PresensiModel->get_hari($yes->tahun, $yes->semester, $this->session->userdata('iddosen'));
        $data['data_matakuliah']  = $this->PresensiModel->get_matakuliah($yes->tahun, $yes->semester, $this->session->userdata('iddosen'));
        $data['data_nama_kelas']  = $this->PresensiModel->get_nama_kelas($yes->tahun, $yes->semester, $this->session->userdata('iddosen'), $this->session->userdata('idkelas'));
        $idpenyelenggaraans       = $this->session->userdata('idpenyelenggaraans_');
        $nama_kelompok            = $nama_kelas;
        $data['data_mhs']         = $this->PresensiModel->get_tampil_mahasiswa_kelompok($idpenyelenggaraans, $nama_kelompok, $this->session->userdata('kls'));
    ...
    public function tampilKrs()
    {
        ...
        $data['idsmt'] = $penyelenggaraan->semester;
        $data['tahun_akademik'] = $penyelenggaraan->tahun_akademik;
        $data['prodi'] = $this->jm->get_prodi();
        $data['kelas'] = $this->jm->get_kelas();
        $data['tampil_data'] = $this->mp->absen($nim, $tahun, $idsmt);
        $this->load->view('layouts/header');
        $this->load->view('layouts/sidebar');
        $this->load->view('presensi/mahasiswa', $data);
        $this->load->view('layouts/footer');
    ...
    {
        $pertemuan = $this->input->post('hidden_id');
        $data['nmatkul'] = $this->input->post('hidden_nmatkul');
        $data['nmdosen'] = $this->input->post('hidden_nmdosen');
        $this->session->set_userdata('pertemuan', $pertemuan);
        $this->config->config["pageTitle"] = 'Data Pertemuan';
        $yes                      = $this->PresensiModel->get_yes();
        $data['sql_jadwal_dosen'] = $this->PresensiModel->get_jadwal_dosen($yes->tahun, $yes->semester, $this->session->userdata('iddosen'));
        $data['data_ruangan']     = $this->PresensiModel->get_ruang();
        ...
        $data['data_kehadiran_pertemuan'] = $this->PresensiModel->get_kehadiran_pertemuan($pertemuan, $idkelas, $this->session->userdata('kls'));

        $this->load->view('layouts/header');
        $this->load->view('layouts/sidebar');
        $this->load->view('presensi/pertemuan', $data);
        $this->load->view('layouts/footer');
    }
}
